...
import { deletePermanentlyPermission } from "@/models/permission/actions";
...
export default function DialogDeletePermission(props: IProps) {
  ...
  const deletePermission = async () => {
    ...
    const result = await deletePermanentlyPermission(permission.id.toString());
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="max-w-lg px-6">
        <DialogHeader>
          ...
          >
            <span className="text-red-500">Xóa quyền hạn</span>
            <span className="text-primary"> - {permission.description}</span>
          </DialogTitle>
          ...
          <DialogDescription className="dark:text-[#f5f5f5] py-4">
            Quyền hạn{" "}
            <span className="font-semibold">{permission.description}</span> sẽ
            bị xóa vĩnh viễn.{" "}
            <span className="font-semibold">Toàn bộ dữ liệu</span> liên quan đến
            quyền hạn này trong cơ sở dữ liệu cũng sẽ bị xóa theo. Chức năng này
